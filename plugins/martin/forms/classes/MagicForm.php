...
use Standard\Positions\Models\Positions;
...
abstract class MagicForm extends ComponentBase {
    ...
    public function onRun() {
        ...
        $this->page['positions'] = Positions::all();
    ...
    public function onFormSubmit() {
        ...
        //job test send questions
        foreach ($post as $key => $value) {
            //unset($post[$key]);
            if (strpos($key, '_') !== false) { //suspected
                if(is_numeric($key[0])){
                    $id = (int)$key[0];
                    $q = Questions::where('question_number',$id)->take(1)->get();
                    if(isset($q[0]->question) && !empty($q[0]->question)){
        ...
        unset($post['_token'], $post['g-recaptcha-response'], $post['_session_key']);
